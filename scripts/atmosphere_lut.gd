...
func _initialize_render(atmosphere_radius_ratio: float, density_falloff: float) -> void:
	...
	if not spirv.compile_error_compute.is_empty():
		...
		return
	_shader = _rendering_device.shader_create_from_spirv(spirv)
	...
	_uniform_set = _rendering_device.uniform_set_create([uniform], _shader, 0)
	...
	_rendering_device.compute_list_bind_compute_pipeline(command_list, _pipeline)
...
func _shutdown_render() -> void:
	...
	for resource in [_buffer, _pipeline, _shader]:
