...
func _process(delta: float) -> void:
	...
	if _floating_origin != null:
		...
		if not shift_total.is_equal_approx(_origin_shift_total):
			...
			queue_redraw()
			return
	var nearest := Gravity.get_nearest_surface(player.global_position) as Node3D
	if nearest == null:
		visible = false
		_body = null
		return
	...
	_altitude = Gravity.get_altitude(player.global_position)
	queue_redraw()
...
func _draw_ship(center: Vector2, radius: float, scale: float) -> void:
	...
	if not is_instance_valid(ship) or Gravity.get_nearest_surface(ship.global_position) != _body:
		return
