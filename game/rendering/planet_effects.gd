...
const OceanShaderPath := "res://game/rendering/shaders/ocean_effect.glsl"
const AtmosphereShaderPath := "res://game/rendering/shaders/atmosphere_effect.glsl"
const BlitShaderPath := "res://game/rendering/shaders/blit_effect.glsl"
const OceanShader := preload("res://game/rendering/shaders/ocean_effect.glsl")
const AtmosphereShader := preload("res://game/rendering/shaders/atmosphere_effect.glsl")
const BlitShader := preload("res://game/rendering/shaders/blit_effect.glsl")
...
const OceanCausticTexture := preload("res://assets/shared/ocean_textures/caustics.png")
const BlueNoiseTexture := preload("res://assets/shared/planet_textures/blue_noise.png")

const MAX_BODIES := 16
const OCEAN_FLOATS := 80
...
func _initialize_resources(rendering_device: RenderingDevice) -> bool:
	if _initialized:
		return true
	for entry in [["ocean", OceanShader], ["atmosphere", AtmosphereShader], ["blit", BlitShader]]:
		var shader_file: RDShaderFile = entry[1]
		if shader_file == null:
			push_error("planet_effects: cannot load shader")
			return false
		var spirv := shader_file.get_spirv()
		if not spirv.compile_error_compute.is_empty():
			push_error("planet_effects: %s" % spirv.compile_error_compute)
			return false
