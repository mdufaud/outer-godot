...
const DustRingShader := preload("res://game/planets/mirage/shaders/dust_ring.gdshader")
...
func _build_rock_meshes() -> Array[ArrayMesh]:
	var topology := PlanetTopologyScript.build_for(ROCK_TOPOLOGY)
	...
	for variant in ROCK_VARIANTS:
		var generator := PlanetHeightGeneratorScript.new(ShapeProfileScript.ASTEROID, 4200 + variant * 137)
		...
		arrays[Mesh.ARRAY_NORMAL] = PlanetTopologyScript.compute_normals(vertices, indices)
...
func _build_dust() -> void:
	...
	_dust_material.shader = DustRingShader
