...
const TrailShader := preload("res://game/planets/watchful_eye/shaders/comet_trail.gdshader")
...
func _ready() -> void:
	...
	_material.set_shader_parameter("steps", TOUCH_STEPS if touch_service.is_touch_ui() else DESKTOP_STEPS)
