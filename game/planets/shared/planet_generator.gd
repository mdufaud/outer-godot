...
const SHADER_PATH := "res://game/planets/shared/shaders/planet_height.comp"
const SHADING_SHADER_PATH := "res://game/planets/shared/shaders/planet_shading.comp"
const PERTURB_SHADER_PATH := "res://game/planets/shared/shaders/planet_perturb.comp"
const PERTURB_SCALE := 50.0
const WORKGROUP_SIZE := 256

static var _shared_pipelines: Dictionary = {}
static var _shared_refcount := 0
const EARTH := PlanetShapeProfileScript.EARTH
const MOON := PlanetShapeProfileScript.MOON
const ALIEN := PlanetShapeProfileScript.ALIEN
const SHATTERED := PlanetShapeProfileScript.SHATTERED
const MOAT := PlanetShapeProfileScript.MOAT
const ASTEROID := PlanetShapeProfileScript.ASTEROID
const GLACIER := PlanetShapeProfileScript.GLACIER
const MIRAGE := PlanetShapeProfileScript.MIRAGE

var _height_kind := EARTH
var _profile_id := PlanetShapeProfileScript.EARTH
var _seed := 0
...
func _init(profile_id: int, seed: int) -> void:
	_profile_id = profile_id
	_height_kind = PlanetShapeProfileScript.height_kind(profile_id)
	_seed = seed
	_build_settings()
...
func _generate_render(positions: PackedByteArray, count: int) -> void:
	...
	var constants := PackedInt32Array([count, _height_kind, _craters.size(), _spike_count]).to_byte_array()
...
func _generate_shading_render(positions: PackedByteArray, count: int) -> void:
	...
	var shading_kind := MOON if _height_kind == MOON else EARTH
...
func _build_shading_settings() -> void:
	...
	match _profile_id:
		PlanetShapeProfileScript.FIERY_TWIN, PlanetShapeProfileScript.CYCLOPS:
			shading_seed = 10
		PlanetShapeProfileScript.ICEY_TWIN:
			shading_seed = -1587
		PlanetShapeProfileScript.TUMBLING_BEAN, PlanetShapeProfileScript.WATCHFUL_EYE:
			shading_seed = 0
	...
	if _height_kind != MOON:
		...
		match _profile_id:
			PlanetShapeProfileScript.FIERY_TWIN:
			...
			PlanetShapeProfileScript.ICEY_TWIN:
				...
				small_scale = 6.34
				small_elevation = 1.0
			PlanetShapeProfileScript.CYCLOPS:
				detail_warp_elevation = 18.72
				detail_elevation = -1.25
				large_elevation = 1.0
				small_scale = 5.0
				small_elevation = 1.0
		var detail_scale := 1.5 if _profile_id == PlanetShapeProfileScript.EARTH else 0.89
		var detail_offset := Vector3.ZERO if _profile_id == PlanetShapeProfileScript.EARTH else Vector3(1.35, 0.0, 0.0)
		_set_shading_simple_noise(3, random, 4, 0.5, 2.0, detail_scale, detail_elevation, 0.0, detail_offset)
		_set_shading_simple_noise(0, random, 4, 0.5, 2.0, 1.0 if _profile_id != PlanetShapeProfileScript.EARTH else 2.96, detail_warp_elevation, 0.0)
		_set_shading_simple_noise(6, random, 4, 0.26 if _profile_id != PlanetShapeProfileScript.EARTH else 0.5, 2.0, 0.425 if _profile_id != PlanetShapeProfileScript.EARTH else 1.52, large_elevation, 0.0)
		_set_shading_simple_noise(9, random, 4 if _profile_id != PlanetShapeProfileScript.EARTH else 5, 0.5 if _profile_id != PlanetShapeProfileScript.EARTH else 0.65, 2.0 if _profile_id != PlanetShapeProfileScript.EARTH else 4.13, small_scale, small_elevation, 0.0)
	else:
		_set_shading_simple_noise(0, random, 4, 0.5, 2.0, 1.875925, 1.1754117, 0.0)
		var detail_warp_elevation := 2.2643394
		if _profile_id == PlanetShapeProfileScript.WATCHFUL_EYE:
		...
		_build_moon_points(10 if _profile_id == PlanetShapeProfileScript.WATCHFUL_EYE else 32, shading_seed)
		# Bright ejecta rays are the strongest "moon" cue, so the comet keeps one.
		_build_ejecta_craters(1 if _profile_id == PlanetShapeProfileScript.WATCHFUL_EYE else 2, 102 if _profile_id == PlanetShapeProfileScript.WATCHFUL_EYE else 0, 9.0 if _profile_id == PlanetShapeProfileScript.WATCHFUL_EYE else 11.0)
